"""
AGENT 2: Results Extractor + RAG Builder (GROQ-POWERED, THREADED)
Uses Groq API (14,400 free requests/day!) instead of Gemini
60-70% faster with threading!
"""
...
def fetch_pmc_fulltext(pmc_id: str) -> Optional[str]:
    """Fetch full-text from PubMed Central (thread-safe)"""
...
def build_ragie_database_threaded(papers: List[Dict], progress: ProgressTracker) -> RagieClient:
    ...
    # STEP 1: Fetch full-texts in parallel
    papers_with_text = []
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for future in as_completed(futures):
            ...
            if is_fulltext:
                fulltext_count += 1
                progress.log(f"  ✓ Full-text: {paper['title'][:40]}... ({len(text.split())} words)", "success")
            else:
                progress.log(f"  ○ Abstract: {paper['title'][:40]}...", "info")
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                uploaded_count += 1
                progress.log(f"  ✓ Uploaded ({uploaded_count}/{len(papers)})", "success")
            except Exception as e:
                progress.log(f"  ✗ Upload error: {str(e)}", "error")
    
    progress.log(f"✓ RAG database built: {uploaded_count} documents ☁️", "success")
...
"""
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for future in as_completed(futures):
            ...
            progress.log(f"  ✓ Extracted: {result['title'][:40]}... ({findings_count} findings)", "success")
...
def extract_and_build_rag(papers: List[Dict], progress_callback: Optional[Callable] = None) -> Dict:
    """
    MAIN FUNCTION: Extract results and build RAG (GROQ-POWERED, THREADED)
    """
    progress = ProgressTracker(progress_callback)
    
    progress.log("=" * 60, "info")
    progress.log("📊 AGENT 2: RAG + EXTRACTOR (Groq, Threaded) ⚡", "info")
    progress.log("=" * 60, "info")
    ...
    progress.log("=" * 60, "info")
    progress.log(f"✅ AGENT 2 COMPLETE: Processed {stats['papers_processed']} papers", "complete")
    progress.log(f"   {stats['total_findings']} findings, powered by Groq ⚡", "info")
    progress.log("=" * 60, "info")
...
if __name__ == "__main__":
    print("This is the GROQ-POWERED, THREADED version of Agent 2")
    print("Run: python run_pipeline_groq.py")
...
#     with ThreadPoolExecutor(max_workers=1) as executor:  # NCBI rate limit: 3/sec
#         futures = [executor.submit(fetch_text_for_paper, paper) for paper in papers]
...
#     with ThreadPoolExecutor(max_workers=1) as executor:  # Gemini rate limit safe
#         futures = [executor.submit(extract_single_paper, paper) for paper in papers]
